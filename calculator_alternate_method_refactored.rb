...
def multiply_or_divide(equation_array, element, index)
  if element == "*"
...
def add_or_subtract(equation_array, element, index)
  if element == "+"
    equation_array[index - 1] = equation_array[index - 1].to_f + 
                                equation_array[index + 1].to_f
    delete_two_elements(equation_array, index)
...
def calculate(equation_array)
  ...
  equation_array.each_with_index do |element, index|
    multiply_or_divide(equation_array, element, index)
  end

  equation_array.each_with_index do |element, index|
    add_or_subtract(equation_array, element, index)  
  end
  equation_array[0]
end
